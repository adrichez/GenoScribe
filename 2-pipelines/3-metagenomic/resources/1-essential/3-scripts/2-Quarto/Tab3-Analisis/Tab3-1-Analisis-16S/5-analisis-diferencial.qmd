...
<div class="informe-resumen">
  ...
  <ul>
    <li>
      <strong>Tablas de abundancia diferencial</strong>: archivos Excel que contienen valores estadísticos relevantes, como logFC, promedio de expresión, estadístico t, valor p y valor p ajustado, para cada taxón identificado. Estas tablas permiten un análisis cuantitativo detallado de los cambios de abundancia entre los grupos.
    </li>
    <li>
      <strong>Gráficos de volcán</strong>: representaciones visuales que facilitan la interpretación rápida de los resultados, mostrando los taxones con cambios significativos en abundancia y su magnitud relativa. Los puntos en los extremos del gráfico destacan los taxones más diferencialmente abundantes.
    </li>
  </ul>
  ...
  <ul>
    <li>
      <strong>5.1. Resultados a nivel de familia</strong>: se muestran las tablas y gráficos correspondientes a las familias microbianas.
    </li>
    <li>
      <strong>5.2. Resultados a nivel de género</strong>: se presentan las tablas y gráficos correspondientes a los géneros microbianos.
    </li>
    <li>
      <strong>5.3. Resultados a nivel de especie</strong>: se incluyen las tablas y gráficos de las especies microbianas identificadas.
    </li>
  </ul>
...
<ul>
  <li>
    <strong>Tablas de abundancia diferencial:</strong> archivos Excel que contienen los cambios de abundancia entre grupos, valores de p y q, fold changes y abundancias medias. Estas tablas permiten identificar de forma precisa qué taxones presentan diferencias significativas.
  </li>
  <li>
    <strong>Gráficas de volcanos:</strong> representaciones visuales de los resultados, donde se muestran los cambios de abundancia (log2 fold change) frente a la significancia estadística (-log10 p-value). Los volcanos facilitan la identificación rápida de los taxones más relevantes en cada contraste.
  </li>
</ul>
...
<ol>
  <li>
    <strong>Resultados a nivel de familia (L5)</strong>: análisis de diferencias de abundancia entre grupos a nivel de familias microbianas.
  </li>
  <li>
    <strong>Resultados a nivel de género (L6)</strong>: análisis a nivel de género, con mayor resolución taxonómica que el nivel de familia.
  </li>
  <li>
    <strong>Resultados a nivel de especie (L7)</strong> (si se dispone de datos): análisis a nivel de especie, ofreciendo la resolución más detallada.
  </li>
</ol>
...
if ("familia" %in% names(folder_files)) {
  ...
  <p>
    ...
    Cada fila de la tabla representa una familia bacteriana identificada, mientras que cada columna muestra los resultados de la comparación entre los grupos definidos en el experimento.
    Para cada comparación se incluye:
  </p>
  ...
  for (i in seq_len(nrow(df_familia))) {
    ...
    if (!is.na(xls_file) && file.exists(xls_file)) {
      ...
      <p style="margin-top: 30px; margin-bottom: 15px;">
        <strong>Tabla de abundancia diferencial:</strong>
      </p>

      <p>
        La tabla interactiva a continuación muestra los valores estadísticos para cada familia identificada, incluyendo logFC, promedio de expresión, estadístico t, valor p y valor p ajustado. 
        Puedes ordenar las columnas, buscar familias específicas y explorar los resultados de manera interactiva.
      </p>
      ...
      out <- tagAppendChild(out, HTML('
      <div class="alert-box" style="margin: 30px 0;">
        ⚠️ <em>No se encontró tabla para esta comparación.</em>
      </div>
      '))
    }
    ...
    if (!is.na(png_file) && file.exists(png_file)) {
      out <- tagAppendChild(out, HTML(glue('
      <p style="margin-top: 35px; margin-bottom: 15px;">
        <strong>Volcán de la comparación:</strong>
      </p>

      <p>
        El gráfico de volcán permite identificar visualmente las familias más diferencialmente abundantes entre los grupos. 
        Los puntos situados en los extremos del gráfico representan familias con cambios significativos en abundancia y alta magnitud de logFC.
      </p>
      ...
      out <- tagAppendChild(out, HTML('
      <div class="alert-box" style="margin: 30px 0;">
        ⚠️ <em>No se encontró gráfica de volcán para esta comparación.</em>.
      </div>
      '))
    }
  }
  ...
  <p>
    En esta subsección se presentan los resultados de las diferencias de abundancia entre grupos a nivel de <strong>familias microbianas</strong>. 
    Cada fila de la tabla debería representar una familia bacteriana identificada, mientras que cada columna mostraría los resultados de la comparación entre los grupos definidos en el experimento.
    Para cada comparación, normalmente se incluyen:
  </p>
  ...
  <div class="alert-box" style="margin: 30px 0;">
    ⚠️ No se encontraron resultados a nivel de familia (carpeta "familia" ausente o vacía).
  </div>
  '))
}
...
if ("genero" %in% names(folder_files)) {
  ...
  <p>
    ...
    Cada fila de la tabla representa un género bacteriano identificado, mientras que cada columna muestra los resultados de la comparación entre los grupos definidos en el experimento.
    Para cada comparación se incluye:
  </p>
  ...
  for (i in seq_len(nrow(df_genero))) {
    ...
    if (!is.na(xls_file) && file.exists(xls_file)) {
      ...
      <p>
        La tabla interactiva a continuación muestra los valores estadísticos para cada género identificado, incluyendo logFC, promedio de expresión, estadístico t, valor p y valor p ajustado. 
        Puedes ordenar las columnas, buscar géneros específicos y explorar los resultados de manera interactiva.
      </p>
    ...
    if (!is.na(png_file) && file.exists(png_file)) {
      ...
      <p>
        El gráfico de volcán permite identificar visualmente los géneros más diferencialmente abundantes entre los grupos. 
  ...
  <p>
    En esta subsección se presentan los resultados de las diferencias de abundancia entre grupos a nivel de <strong>géneros microbianos</strong>. 
    Cada fila de la tabla debería representar un género bacteriano identificado, mientras que cada columna mostraría los resultados de la comparación entre los grupos definidos en el experimento.
    Para cada comparación, normalmente se incluyen:
  </p>
...
if ("especie" %in% names(folder_files)) {
  ...
  <p>
    ...
    Cada fila de la tabla representa una especie bacteriana identificada, mientras que cada columna muestra los resultados de la comparación entre los grupos definidos en el experimento.
    Para cada comparación se incluye:
  </p>
  ...
  for (i in seq_len(nrow(df_especie))) {
    ...
    if (!is.na(xls_file) && file.exists(xls_file)) {
      ...
      <p>
        La tabla interactiva a continuación muestra los valores estadísticos para cada especie identificada, incluyendo logFC, promedio de expresión, estadístico t, valor p y valor p ajustado. 
        Puedes ordenar las columnas, buscar especies específicas y explorar los resultados de manera interactiva.
      </p>
    ...
    if (!is.na(png_file) && file.exists(png_file)) {
      ...
      <p>
        El gráfico de volcán permite identificar visualmente las especies más diferencialmente abundantes entre los grupos. 
  ...
  <p>
    En esta subsección se presentan los resultados de las diferencias de abundancia entre grupos a nivel de <strong>especies microbianas</strong>. 
    Cada fila de la tabla debería representar una especie bacteriana identificada, mientras que cada columna mostraría los resultados de la comparación entre los grupos definidos en el experimento.
    Para cada comparación, normalmente se incluyen:
  </p>
  ...
  <div class="alert-box" style="margin: 30px 0;">
    ⚠️ No se encontraron resultados a nivel de especie (carpeta "especie" ausente o vacía).
  </div>
  '))
}
