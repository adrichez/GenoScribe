...
<ul style="list-style-type: disc; padding-left: 20px;">
  <li style="margin-bottom: 1em;">
    <strong>Rarefacción y cobertura:</strong> se generan curvas que muestran cómo aumenta el número de especies observadas a medida que incrementamos el número de secuencias analizadas. Estas curvas son una herramienta clave para evaluar si la profundidad de muestreo es suficiente. 
    <ul>
  ...
  <li style="margin-bottom: 1em;">
    <strong>Diversidad alfa:</strong> hace referencia a la diversidad dentro de cada muestra individual. Existen distintos índices que capturan diferentes aspectos:
    <ul>
      <li><em>Riqueza observada:</em> número total de taxones detectados en la muestra.</li>
      <li><em>Índice de Shannon:</em> combina riqueza y equidad, reflejando tanto cuántos taxones hay como qué tan uniformes son sus abundancias.</li>
      <li><em>Equidad (evenness):</em> mide la uniformidad en la distribución de abundancias; una comunidad con todos los taxones presentes en proporciones similares es más equitativa que una dominada por pocos.</li>
      <li><em>Diversidad filogenética de Faith:</em> añade la dimensión evolutiva, considerando la distancia filogenética entre los organismos detectados.</li>
    </ul>
  ...
  <li style="margin-bottom: 1em;">
    <strong>Diversidad beta:</strong> estudia las diferencias entre comunidades, comparando cómo varía la composición microbiana entre las distintas muestras o grupos de estudio. Se basa en cálculos de distancia o disimilitud, entre los que destacan:
    <ul>
      <li><em>Bray–Curtis:</em> sensible a las abundancias relativas de los taxones.</li>
      <li><em>Jaccard:</em> centrado únicamente en presencia o ausencia de taxones.</li>
      <li><em>UniFrac no ponderado:</em> incorpora la filogenia pero sin considerar las abundancias.</li>
      <li><em>UniFrac ponderado:</em> añade información de abundancia a la distancia filogenética.</li>
    </ul>
...
\n\n'))
  ...
  cat(glue::glue('
  <div class="alert-box">
    ⚠️ El archivo <code>alpha-rarefaction.qzv</code> no existe en la ruta especificada: 
    <code>{ruta_alpha_rarefaction_qzv}</code>
  </div>
\n\n'))
}
...
<ul>
  <li><strong>Diversidad de Shannon</strong>: refleja simultáneamente riqueza y equidad.</li>
  <li><strong>Número de características observadas</strong>: mide la riqueza como número de taxones detectados.</li>
  <li><strong>Diversidad filogenética de Faith</strong>: incorpora la distancia evolutiva entre los taxones observados.</li>
</ul>
...
<ul>
  <li><code>shannon.csv</code>: valores del índice de Shannon a diferentes profundidades de rarefacción para cada muestra.</li>
  <li><code>observed_features.csv</code>: número de taxones observados en cada punto de rarefacción.</li>
  <li><code>faith_pd.csv</code>: estimación de diversidad filogenética de Faith a lo largo de las profundidades de muestreo.</li>
</ul>
...
if (file.exists(ruta_shannon_csv)) {
  ...
  cat(glue::glue('
  <div class="alert-box">
    ⚠️ El archivo <code>shannon.csv</code> no existe en la ruta especificada: 
...
if (file.exists(ruta_shannon_csv)) {
  ...
  <p style="text-align:center;">
    <a href="{ruta_shannon_csv}" target="_blank" class="boton-file">
      💾 Descargar archivo "shannon.csv" original
    </a>
  </p>
  \n\n'))
}
...
if (file.exists(ruta_observed_features_csv)) {
  ...
  cat(glue::glue('
  <div class="alert-box">
    ⚠️ El archivo <code>observed_features.csv</code> no existe en la ruta especificada: 
...
if (file.exists(ruta_observed_features_csv)) {
  ...
  <p style="text-align:center;">
    <a href="{ruta_observed_features_csv}" target="_blank" class="boton-file">
      💾 Descargar archivo "observed_features.csv" original
    </a>
  </p>
  \n\n'))
}
...
if (file.exists(ruta_faith_pd_csv)) {
  ...
  cat(glue::glue('
  <div class="alert-box">
    ⚠️ El archivo <code>faith_pd.csv</code> no existe en la ruta especificada: 
...
if (file.exists(ruta_faith_pd_csv)) {
  ...
  <p style="text-align:center;">
    <a href="{ruta_faith_pd_csv}" target="_blank" class="boton-file">
      💾 Descargar archivo "faith_pd.csv" original
    </a>
  </p>
  \n\n'))
}
...
\n\n'))
  ...
  cat(glue::glue('
  <div class="alert-box">
    ⚠️ La carpeta <code>core-metrics-results/code> no existe en la ruta especificada: 
    <code>{ruta_core_metrics_results}</code>
  </div>
\n\n'))
}
...
<ul>
  <li><code>.qza</code>: archivos de datos que contienen los valores calculados por muestra (vectores numéricos de diversidad).</li>
  <li><code>.qzv</code>: archivos de visualización que permiten explorar de forma interactiva los resultados mediante boxplots y pruebas estadísticas de significancia entre grupos definidos en la metadata.</li>
</ul>
...
<ul>
  <li><strong>Shannon</strong>: índice que combina riqueza y equidad, proporcionando una medida de la diversidad efectiva que penaliza la dominancia de unos pocos taxones.</li>
  <li><strong>Faith PD</strong>: incorpora información filogenética, midiendo la suma de las longitudes de las ramas del árbol evolutivo presentes en cada muestra.</li>
  <li><strong>Evenness</strong>: refleja el grado de uniformidad en la distribución de abundancias; valores altos implican que los taxones tienen abundancias similares.</li>
  <li><strong>Observed features</strong>: mide la riqueza pura, es decir, el número de taxones distintos observados en la muestra sin considerar abundancia relativa ni filogenia.</li>
</ul>
...
<ul>
  <li><code>shannon_vector.qza</code> y <code>shannon-group-significance.qzv</code> (índice de Shannon y su comparación entre grupos).</li>
  <li><code>faith_pd_vector.qza</code> y <code>faith-pd-group-significance.qzv</code> (diversidad filogenética de Faith y comparaciones entre grupos).</li>
  <li><code>evenness_vector.qza</code> y <code>evenness-group-significance.qzv</code> (evenness por muestra y visualización por grupos).</li>
  <li><code>observed_features_vector.qza</code> y <code>observed-features-group-significance.qzv</code>(riqueza observada y su visualización por grupos).</li>
</ul>
...
if (file.exists(indices_html$`shannon-group-significance`)) {
  ...
  <div class="alert-box">
    ...
    <span style="font-size:14px; font-weight:normal;">
      No se encontró el archivo <code>index.html</code> correspondiente. <br>
      Asegúrese de haber generado y descomprimido el archivo <code>shannon-group-significance.qzv</code>.
    </span>
  </div>
  \n\n'))
}
...
if (file.exists(indices_html$`faith-pd-group-significance`)) {
  ...
  cat(glue::glue('
  <div class="alert-box">
    ❌ <span style="font-size:18px;">La visualización no está disponible</span><br>
    <span style="font-size:14px; font-weight:normal;">
      No se encontró el archivo <code>index.html</code> correspondiente. <br>
      Asegúrese de haber generado y descomprimido el archivo <code>faith-pd-group-significance.qzv</code>.
    </span>
  </div>
  \n\n'))
}
...
if (file.exists(indices_html$`evenness-group-significance`)) {
  ...
  <div class="alert-box">
    ...
    <span style="font-size:14px; font-weight:normal;">
      No se encontró el archivo <code>index.html</code> correspondiente. <br>
      Asegúrese de haber generado y descomprimido el archivo <code>evenness-group-significance.qzv</code>.
    </span>
  </div>
  \n\n'))
}
...
if (file.exists(indices_html$`observed-features-group-significance`)) {
  ...
  <div class="alert-box">
    ...
    <span style="font-size:14px; font-weight:normal;">
      No se encontró el archivo <code>index.html</code> correspondiente. <br>
      Asegúrese de haber generado y descomprimido el archivo <code>observed-features-group-significance.qzv</code>.
    </span>
  </div>
  \n\n'))
}
...
<ul style="list-style-type: disc; padding-left: 20px;">
  <li style="margin-bottom: 1em;">
    <p><strong>Distancia de Bray–Curtis</strong>: esta métrica evalúa las diferencias entre comunidades basándose en las abundancias relativas de los taxones presentes en cada muestra. Es especialmente útil para detectar cambios en la composición cuando la abundancia de ciertos taxones domina la comunidad.</p>
    <ul style="list-style-type: none; padding-left: 20px;">
      <li>→ <strong>Archivos de visualización</strong>: <code>bray_curtis_emperor.qzv</code>, <code>bray-curtis-emperor-Type.qzv</code></li>
      <li>→ <strong>Archivos numéricos</strong>: <code>bray_curtis_distance_matrix.qza</code>, <code>bray_curtis_pcoa_results.qza</code></li>
    </ul>
  </li>

  <li style="margin-bottom: 1em;">
    <p><strong>Distancia de Jaccard</strong>: mide la disimilitud entre comunidades considerando únicamente la presencia o ausencia de taxones, ignorando sus abundancias. Es útil para identificar cambios en la composición de especies sin que influyan los taxones dominantes.</p>
    <ul style="list-style-type: none; padding-left: 20px;">
      <li>→ <strong>Archivos de visualización</strong>: <code>jaccard_emperor.qzv</code></li>
      <li>→ <strong>Archivos numéricos</strong>: <code>jaccard_distance_matrix.qza</code>, <code>jaccard_pcoa_results.qza</code></li>
    </ul>
  </li>

  <li style="margin-bottom: 1em;">
    <p><strong>Unweighted UniFrac</strong>: evalúa la diversidad filogenética considerando la presencia o ausencia de taxones. Tiene en cuenta la distancia evolutiva entre taxones, lo que permite detectar diferencias filogenéticas entre comunidades.</p>
    <ul style="list-style-type: none; padding-left: 20px;">
      <li>→ <strong>Archivos de visualización</strong>: <code>unweighted_unifrac_emperor.qzv</code>, <code>unweighted-unifrac-emperor-Type.qzv</code>, <code>unweighted-unifrac-Type-significance.qzv</code></li>
      <li>→ <strong>Archivos numéricos</strong>: <code>unweighted_unifrac_distance_matrix.qza</code>, <code>unweighted_unifrac_pcoa_results.qza</code></li>
    </ul>
  </li>

  <li style="margin-bottom: 1em;">
    <p><strong>Weighted UniFrac</strong>: combina información filogenética y abundancia relativa de taxones, proporcionando una visión más completa de las diferencias entre comunidades al considerar tanto la presencia/ausencia como la proporción de cada taxón.</p>
    <ul style="list-style-type: none; padding-left: 20px;">
      <li>→ <strong>Archivos de visualización</strong>: <code>weighted_unifrac_emperor.qzv</code></li>
      <li>→ <strong>Archivos numéricos</strong>: <code>weighted_unifrac_distance_matrix.qza</code>, <code>weighted_unifrac_pcoa_results.qza</code></li>
    </ul>
  </li>
</ul>
...
if (file.exists(indices_html$`bray_curtis_emperor`)) {
  ...
  cat(glue::glue('
  <div class="alert-box">
    ❌ <span style="font-size:18px;">Visualización no disponible</span><br>
    <span style="font-size:14px; font-weight:normal;">
      No se encontró el archivo <code>index.html</code> correspondiente. <br>
      Asegúrese de haber generado y descomprimido <code>bray_curtis_emperor.qzv</code>.
    </span>
  </div>
  \n\n'))
}
...
if (file.exists(indices_html$`bray-curtis-emperor-Type`)) {
  ...
  <div class="alert-box">
    ...
    <span style="font-size:14px; font-weight:normal;">
      No se encontró el archivo <code>index.html</code> correspondiente. <br>
      Asegúrese de haber generado y descomprimido <code>bray-curtis-emperor-Type.qzv</code>.
    </span>
  </div>
  \n\n'))
}
...
<ul style="list-style-type: none; padding-left: 20px;">
  <li>→ <strong><code>bray_curtis_distance_matrix.qza</code></strong>: matriz de distancias entre todas las muestras, donde cada valor representa la disimilitud de Bray–Curtis entre un par de comunidades.</li>
  <li>→ <strong><code>bray_curtis_pcoa_results.qza</code></strong>: resultados numéricos del análisis PCoA, incluyendo las coordenadas de cada muestra en el espacio multivariado y la varianza explicada por cada eje.</li>
</ul>
...
if (file.exists(indices_html$`jaccard_emperor`)) {
  ...
  <div class="alert-box">
    ...
    <span style="font-size:14px; font-weight:normal;">
      No se encontró el archivo <code>index.html</code> correspondiente. <br>
      Asegúrese de haber generado y descomprimido <code>jaccard_emperor.qzv</code>.
    </span>
  </div>
  \n\n'))
}
...
<ul style="list-style-type: none; padding-left: 20px;">
  <li>→ <strong><code>jaccard_distance_matrix.qza</code></strong>: matriz de distancias basada en la métrica de Jaccard, donde cada valor representa el grado de disimilitud entre un par de muestras.</li>
  <li>→ <strong><code>jaccard_pcoa_results.qza</code></strong>: resultados numéricos del análisis PCoA, incluyendo las coordenadas de cada muestra en el espacio multivariado y el porcentaje de varianza explicada por cada eje.</li>
</ul>
...
```{r, results='asis'}
# Incrustar index.html si está disponible
if (file.exists(indices_html$`unweighted_unifrac_emperor`)) {
  ...
  <p style="text-align:center;">
    <a href="{indices_html$`unweighted_unifrac_emperor`}" target="_blank" class="boton-iframe">
      📊 Abrir visualización Unweighted UniFrac en nueva pestaña
    </a>
  </p>
  ...
  <div class="alert-box">
    ❌ <span style="font-size:18px;">Visualización no disponible</span><br>
    <span style="font-size:14px; font-weight:normal;">
      No se encontró el <code>index.html</code> de <code>unweighted_unifrac_emperor.qzv</code>. 
      Asegúrese de haber descomprimido el archivo <code>.qzv</code>.
    </span>
  </div>
  \n\n'))
}
...
if (file.exists(indices_html$`unweighted-unifrac-emperor-Type`)) {
  ...
  <p style="text-align:center;">
    <a href="{indices_html$`unweighted-unifrac-emperor-Type`}" target="_blank" class="boton-iframe">
      📊 Abrir Unweighted UniFrac (Type) en nueva pestaña
    </a>
  </p>
  ...
  <div class="alert-box">
    ❌ <span style="font-size:18px;">Visualización no disponible</span><br>
    <span style="font-size:14px; font-weight:normal;">
      No se encontró el <code>index.html</code> de <code>unweighted-unifrac-emperor-Type.qzv</code>. 
      Asegúrese de haber generado y descomprimido correctamente el archivo.
    </span>
  </div>
  \n\n'))
}
...
if (file.exists(indices_html$`unweighted-unifrac-Type-significance`)) {
  ...
  <p style="text-align:center;">
    <a href="{indices_html$`unweighted-unifrac-Type-significance`}" target="_blank" class="boton-iframe">
      📊 Abrir resultados de significancia (Unweighted UniFrac) en nueva pestaña
    </a>
  </p>
  ...
  cat(glue::glue('
  <div class="alert-box">
    ⚠️ <span style="font-size:14px;">No se encontró la visualización de significancia.</span><br>
...
<ul style="list-style-type: none; padding-left: 20px;">
  <li>→ <strong><code>unweighted_unifrac_distance_matrix.qza</code></strong>: matriz de distancias Unweighted UniFrac entre todas las muestras (cada celda es la disimilitud filogenética por presencia/ausencia).</li>
  <li>→ <strong><code>unweighted_unifrac_pcoa_results.qza</code></strong>: resultados del PCoA (coordenadas de cada muestra y varianza explicada por ejes), útiles para análisis cuantitativos y exportación de tablas.</li>
</ul>
...
if (file.exists(indices_html$`weighted_unifrac_emperor`)) {
  ...
  <p style="text-align:center;">
    <a href="{indices_html$`weighted_unifrac_emperor`}" target="_blank" class="boton-iframe">
      📊 Abrir Weighted UniFrac (Emperor) en nueva pestaña
    </a>
  </p>
  ...
  <div class="alert-box">
    ❌ <span style="font-size:18px;">Visualización no disponible</span><br>
    <span style="font-size:14px; font-weight:normal;">
      No se encontró el <code>index.html</code> de <code>weighted_unifrac_emperor.qzv</code>. 
      Asegúrese de haber descomprimido el archivo <code>.qzv</code> en la carpeta de <code>unzip</code>.
    </span>
  </div>
  \n\n'))
}
...
<ul style="list-style-type: none; padding-left: 20px;">
  <li>→ <strong><code>weighted_unifrac_distance_matrix.qza</code></strong>: matriz de distancias Weighted UniFrac entre todas las muestras (cada celda representa la disimilitud filogenética ponderada por abundancia entre un par de muestras).</li>
  <li>→ <strong><code>weighted_unifrac_pcoa_results.qza</code></strong>: resultados PCoA con las coordenadas de cada muestra y la varianza explicada por cada eje; útil para extraer tablas de coordenadas y para gráficas personalizadas en R.</li>
</ul>
