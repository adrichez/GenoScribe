...
<p>
  Para garantizar la <strong>reproducibilidad</strong> y <strong>modularidad</strong> del análisis, <code>miARma‑Seq</code> utiliza un archivo de configuración con extensión <code>.ini</code>. Este archivo permite especificar todos los parámetros necesarios para ejecutar el pipeline completo de RNA-Seq, sin necesidad de escribir comandos complejos en la línea de comandos.  
  En el contexto de este informe de metagenómica, se ha utilizado únicamente el módulo de control de calidad de lecturas (<code>FASTQ</code>), pero el archivo <code>.ini</code> podría activar otros pasos del pipeline si se deseara.
</p>
...
<p>
  ...
  En futuras extensiones del análisis metagenómico, se podría aprovechar más módulos del pipeline de miARma‑Seq si se integran otros tipos de datos o se desea automatizar pasos adicionales.
</p>
