...
COPY 3-docker/env_bulk_rna_seq.yml /tmp/env_bulk_rna_seq.yml
...
COPY 1-app /project/1-app
COPY 2-pipelines /project/2-pipelines
COPY 3-docker /project/3-docker
COPY 4-launch /project/4-launch
COPY 5-info /project/5-info
COPY README.md /project/README.md

# 12. Limpiar caches de Quarto y R que arrastran rutas absolutas del host
RUN rm -rf /project/1-app/_freeze /project/1-app/_cache \
    && rm -rf /project/2-pipelines/1-bulk-rna-seq/_freeze /project/2-pipelines/1-bulk-rna-seq/_cache \
    && rm -rf /project/2-pipelines/3-metagenomic/_freeze /project/2-pipelines/3-metagenomic/_cache \
    && find /project -type f -name "*.rds" -delete

# 13. Crear enlace simbólico para que Shiny sirva la app
RUN ln -s /project/1-app /srv/shiny-server/app

# 14. Crear carpeta de reports dentro de www
RUN mkdir -p /srv/shiny-server/app/www/reports

# 15. Permisos de Shiny y entrypoint
RUN chown -R shiny:shiny /srv/shiny-server /project
RUN chmod +x /project/3-docker/entrypoint.sh

# 16. Exponer el puerto 3838
EXPOSE 3838

# 17. Comando por defecto
CMD ["/project/3-docker/entrypoint.sh"]
